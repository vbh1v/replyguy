function getModels(apiKey, provider) {
  let endpoint;
  ...
  getModels(apiKey, provider)
    .then((response) => {
      ...
      chrome.storage.local.get(["openai-model"]).then((model) => {
        ...
        defaultOption.value =
          model["openai-model"] ||
          (provider === "deepseek" ? "deepseek-chat" : "gpt-3.5-turbo");
        defaultOption.text =
          model["openai-model"] ||
          (provider === "deepseek" ? "deepseek-chat" : "gpt-3.5-turbo");
...
document.addEventListener("DOMContentLoaded", function () {
  // Provider select save
  document
    .getElementById("provider-select")
    .addEventListener("change", function () {
      const provider = document.getElementById("provider-select").value;
      chrome.storage.local.set({ provider: provider }).then(() => {
    ...
    .addEventListener("click", function (event) {
      ...
      if (isChecked) {
        chrome.storage.local
          .set({ "automatic-window-close": true })
          .then(() => {
            console.log("Automatic window close enabled");
          });
      } else {
        chrome.storage.local
          .set({ "automatic-window-close": false })
          .then(() => {
  ...
  chrome.storage.local.get(["automatic-window-close"]).then((result) => {
    if (result["automatic-window-close"] == undefined) {
      ...
      document.getElementById("window-close").checked =
        result["automatic-window-close"];
    }
  });
  ...
  chrome.commands.getAll().then((commands) => {
    ...
    commands.forEach((shortcut) => {
      ...
      shortcutElement.innerHTML = `
        <span class="shortcut-key">${shortcut.shortcut || "undefined"}</span> ${
  ...
  const extensionShortcutsButton = document.getElementById(
    "extension-shortcuts-button"
  );
  extensionShortcutsButton.addEventListener("click", function () {
    chrome.tabs.create({ url: "chrome://extensions/shortcuts" });
  });

  // Set provider by default if already there
